...
from nsob.fields.nsob_field_hit import NsobHitField
from nerfstudio.field_components.encodings import Encoding, Identity
from nerfstudio.data.scene_box import SceneBox

@dataclass
class NsobjectModelConfig(ModelConfig):
    """Nerfacto Model Config"""

    _target: Type = field(default_factory=lambda: NsobjectModel)
...
class NsobjectModel(Model):
    ...
    def populate_modules(self):
        ...
        self.field_object = NerfactoField(
            self.scene_box.aabb,
            hidden_dim=self.config.hidden_dim,
            num_levels=self.config.num_levels,
            max_res=self.config.max_res,
    ...
    def get_param_groups(self) -> Dict[str, List[Parameter]]: #key function
        param_groups = {}
    ...
    # TODO: crarify this function between 2 field
    ...
    def get_outputs(self, ray_bundle: RayBundle, batch: Optional[Dict[str, torch.Tensor]]=None):
        ...
        self.outputs_scene = outputs
        outputs_object = self.get_outputs_object(ray_bundle, batch)
        outputs.update(outputs_object)

        return outputs
    
    def get_outputs_object(self, ray_bundle: RayBundle, batch: Optional[Dict[str, torch.Tensor]]=None):
        ray_samples: RaySamples
        ray_samples, weights_list, ray_samples_list = self.proposal_sampler_object(ray_bundle, density_fns=self.density_fns_object)
        field_outputs = self.field_object.forward(ray_samples, compute_normals=self.config.predict_normals)
        if self.config.use_gradient_scaling:
            field_outputs = scale_gradients_by_distance_squared(field_outputs, ray_samples)

        occlusion_mask = None
        # print("Step in trainer:", self.current_step)
        if self.training and batch and (3000 < self.current_step < 10000):
            # print("Num wait till guard mask:", self.config.steps_per_eval_batch)
            pass_through_mask = batch["mask"].unsqueeze(1).expand(ray_samples.frustums.get_centers_on_ray().shape)
            frustum_bound_th = 0
            occlusion_mask = (self.outputs_scene["depth"].unsqueeze(1).expand(ray_samples.frustums.get_centers_on_ray().shape) + frustum_bound_th) < ray_samples.frustums.get_centers_on_ray()
            # occlusion_mask = (self.outputs_scene["depth"].unsqueeze(1).expand(ray_samples.frustums.get_centers_on_ray().shape) + frustum_bound_th) < torch.zeros_like(ray_samples.frustums.get_centers_on_ray())
            occlusion_mask[pass_through_mask] = False
        ...
        outputs = {
            "rgb_object": rgb,
            "accumulation_object": accumulation,
            "depth_object": depth,
        }
        ...
        if self.config.predict_normals:
            normals = self.renderer_normals(normals=field_outputs[FieldHeadNames.NORMALS], weights=weights)
            pred_normals = self.renderer_normals(field_outputs[FieldHeadNames.PRED_NORMALS], weights=weights)
            outputs["normals_object"] = self.normals_shader(normals)
            outputs["pred_normals_object"] = self.normals_shader(pred_normals)
        # These use a lot of GPU memory, so we avoid storing them for eval.
        if self.training:
            outputs["weights_list_object"] = weights_list
            outputs["ray_samples_list_object"] = ray_samples_list

        if self.training and self.config.predict_normals:
            outputs["rendered_orientation_loss_object"] = orientation_loss(
                weights.detach(), field_outputs[FieldHeadNames.NORMALS], ray_bundle.directions
            )

            outputs["rendered_pred_normal_loss_object"] = pred_normal_loss(
                weights.detach(),
                field_outputs[FieldHeadNames.NORMALS].detach(),
                field_outputs[FieldHeadNames.PRED_NORMALS],
            )
        ...
        for i in range(self.config.num_proposal_iterations):
            outputs[f"prop_depth_{i}_object"] = self.renderer_depth(weights=weights_list[i], ray_samples=ray_samples_list[i])
    ...
    def get_metrics_dict(self, outputs, batch):
        ...
        metrics_dict_object = self.get_metrics_dict_object(outputs, batch)
        metrics_dict.update(metrics_dict_object)
        return metrics_dict
    
    def get_metrics_dict_object(self, outputs, batch): # test here
        metrics_dict = {}
        gt_rgb = batch["image"].to(self.device)  # RGB or RGBA image
        mask = batch["mask"].to(self.device)
        # gt_rgb = ~(mask) + (mask * gt_rgb)
        gt_rgb = gt_rgb[mask.flatten()]
        gt_rgb = self.renderer_rgb.blend_background(gt_rgb)  # Blend if RGBA
        # predicted_rgb = outputs["rgb_object"]
        predicted_rgb = outputs["rgb_object"][mask.flatten()]
        metrics_dict["psnr_object"] = self.psnr(predicted_rgb, gt_rgb)
    ...
    def get_loss_dict(self, outputs, batch, metrics_dict=None):
        ...
        loss_dict.update(loss_dict_object)
        return loss_dict
    ...
    def get_loss_dict_object(self, outputs, batch, metrics_dict=None):
        ...
        loss_dict["accumulation_loss_object"] = 10 * self.accumulation_loss(mask.float(), outputs["accumulation_object"])
    ...
    def get_image_metrics_and_images_object(
        ...
        gt_rgb = batch["image"].to(self.device)
        mask = batch["mask"].to(self.device)
        gt_rgb = ~(mask) + (mask * gt_rgb) # set background color white 1 to match output
        gt_rgb = self.renderer_rgb.blend_background(gt_rgb)
    ...
    def forward(self, ray_bundle: RayBundle, batch: Dict[str, torch.Tensor] = None, step: int = None) -> Dict[str, Union[torch.Tensor, List]]:
        if self.collider is not None:
            ray_bundle = self.collider(ray_bundle)
        if step:
            self.current_step = step
        if batch:
            return self.get_outputs(ray_bundle=ray_bundle, batch=batch)
        else:
